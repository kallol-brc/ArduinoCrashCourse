...
//************************************************************/
int _pinD10 = 10; // pin 10 to receive input from sensor
int _pinD7 = 7; // pin 7 to send signal to relay
int _soundLevel = HIGH; // variable for storing output of sensor
boolean _bRelayState = false; // start by assuming OFF state
unsigned long _lastSoundEventTime; // variable to store the time  
                                   // of the last sound event
int _gapBetween2Sounds = 1000; // gap between 2 sound events
...
void loop ()
{
  ...
  {
    if (_bRelayState == false) // if relay is in OPEN state
    {
      //if event is after at least 1 second of the previous event
